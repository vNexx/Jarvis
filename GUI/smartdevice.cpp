...
SmartDevice::SmartDevice(QWidget *parent) : QWidget(parent)
{
...
SmartDevice::SmartDevice(QWidget *parent, const QString& groupName, const QString& deviceName, int id) : SmartDevice(parent)
{
    ...
    if(groupName.isEmpty())
        QMessageBox::information(this, QString("warning"), QString("Error. Empty group name"));
    else
        this->groupName = groupName;

    if(deviceName.isEmpty())
        QMessageBox::information(this, QString("warning"), QString("Error. Empty device name"));
    else
        this->deviceName = deviceName;

    if(id <= 0)
        QMessageBox::information(this, QString("warning"), QString("Error. Invalid device ID"));
    else
        deviceID = id;
}
...
bool SmartDevice::setGroupName(const QString& name)
{
    ...
    else
    {
        ...
        {
            groupName = temp;
            QMessageBox::information(this, QString("warning"), QString("Error. Cannot upload config"));
            return false;
        }
